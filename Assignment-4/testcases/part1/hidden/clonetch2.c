...
/*2 threads created and it executes 2 different thread funcs*/
...
int main(u64 arg1, u64 arg2, u64 arg3, u64 arg4, u64 arg5)
{
  ...
  stackp[0] = mmap(NULL, 8192, PROT_READ|PROT_WRITE, 0);
  if(!stackp[0] || stackp[0] == MAP_ERR){
        printf("Can not allocated stack\n");
        exit(0);
  }
  thpid[0] = clone(&thsum, ((u64)stackp[0]) + 8192, (void *)&tharg[0]);   // Returns the PID of the thread
  if(thpid[0] <= 0){
       printf("Error creating thread!\n");
       exit(0);
  }
  stackp[1] = mmap(NULL, 8192, PROT_READ|PROT_WRITE, 0);
  if(!stackp[1] || stackp[1] == MAP_ERR){
        printf("Can not allocated stack\n");
        exit(0);
  }
  thpid[1] = clone(&thmul, ((u64)stackp[1]) + 8192, (void *)&tharg[1]);   // Returns the PID of the thread
  if(thpid[1] <= 0){
       printf("Error creating thread!\n");
       exit(0);
  }
